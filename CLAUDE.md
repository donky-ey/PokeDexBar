...
- **여러 조건줄을 우선순위로 접을 때, 그 줄들이 서로 다른 대상의 것일 수 있다 — 그리고 응답은
  ...
  **두 번 틀렸고, 두 번째가 더 나빴다.** ① 처음엔 "응답에는 누구 것인지가 안 적혀 있다"고 적고
  휴리스틱(`levelLivesInAnotherDetail`)으로 풀었다 — 도구 줄만 덮어서 조건이 *친밀도*인 지방 모습에는
  방어가 없었고, **관동 나옹이 알로라의 친밀도를 물려받았다**(사용자 제보). ② 그래서 "응답이 알려
  준다"로 고치면서 그 필드를 **`base_form` 이라고 적었는데, 응답에서 그 필드는 전부 null 이다** —
  전 540체인 576줄 전수 확인(2026-09-22). 줄의 주인은 **`required_pokemon_form`** 에 적힌다.

  결과가 무서웠다: `isRegional` 이 **한 번도 참이 된 적이 없어** 지방 판정 전체가 죽은 코드였고,
  26갈래가 전부 지방 조건을 원종으로 흘렸다 — **기본 야도란이 가라두구팔찌를 요구**했다(사용자 제보).
  야돈·나옹·모래두지·식스테일·가디·찌리리공·디그다·꼬마돌·꼬렛·우파·조로아·질뻐기 등.

  **조사 자체는 옳았는데 필드 이름만 틀리게 옮겨 적었다.** 그때 적은 분포(이브이 여덟 갈래, 피카츄·
  플라베베·루가루암·바스라오가 원종 자신을 가리킨다)는 `required_pokemon_form` 의 실제 분포와 정확히
  일치한다. **요약을 문서에 옮길 때 필드 이름을 응답에 한 번 더 대조하라** — 요약이 맞아도 이름이
  틀리면 코드가 없는 필드를 읽는다.

  **값이 있다고 지방 모습이 아니다.** 576줄 중 81줄에 값이 있는데 가장 많은 것이 이브이(19줄)이고,
  피카츄·플라베베·루가루암·바스라오·호박귀·사탕훔침처럼 지방과 무관한 폼이 대부분이다. 값의 유무로
  가르면 이브이가 조건을 통째로 잃는다 → 판정은 **지방 접미사**로 한다(`PokeAPIClient.isRegional`,
  `Region` 을 단일 소스로 공유). 그리고 지방 줄밖에 없는 갈래(나이킹·창치·마임꽁꽁 등)는 갈라내고
  끝내면 조건이 0개가 되므로 그 줄을 원종 조건으로도 쓴다.

  **한 번도 발동한 적 없는 가드는 테스트가 초록이어도 죽어 있다.** 이 가드가 두 릴리스를 그렇게
  살아남았다. 방어를 새로 만들면 "이게 실제 데이터에서 몇 번 참이 되나"를 한 번 세어 봐라 — 0이면
  방어가 아니라 장식이다.

  **테스트 공백의 정체가 둘이다:** ① 조건 파서 테스트가 전부 **조건줄 하나짜리 픽스처**였다 —
  줄이 둘인 모양을 안 물어봤으니 우선순위가 무엇을 버리는지 물어볼 수조차 없었다. ② 모래두지
  픽스처에 **응답에 없는 `base_form` 값을 손으로 써 넣고 "실제 응답 확인"이라 주석까지 달았다** —
  픽스처가 파서와 같은 오해를 공유해 둘 다 틀린 채로 통과했다(§외부 로그 포맷은 상위 소스의 writer 로 검증한다 와 같은 부류. 그 규칙이
  로그 파서에만 적용된다고 읽지 마라 — **모든 외부 응답**에 해당한다). 외부 응답을 접는 코드에는
  줄이 여럿인 픽스처를 두고, **그 픽스처는 실제 응답에서 잘라 온다**.
- **같은 필드가 "막는 조건"일 수도 "가르는 축"일 수도 있다 — 값의 존재가 아니라 값의 *집합*을 세라.**
  진화 조건줄의 `gender` 는 여섯 갈래에서 진짜 제한이지만(세꼬비는 암컷만 비퀸이 된다), 세 갈래
  (맛보돈·배쓰나이·냐스퍼)에서는 암수 두 줄이 **같은 자식 종**을 가리키고 갈리는 것은 폼뿐이다
  (`oinkologne-male`/`oinkologne-female`). 파서가 "처음 만난 성별"을 제한으로 읽어 셋 다 수컷 줄이
  먼저인 탓에 **암컷이 전부 진화를 못 했다**(사용자 제보: 맛보돈 암컷). → 값이 하나로 모일 때만
  제한이고, 둘 다 나오면 제한이 아니다(`PokeAPIClient.gender(from:)`).
  **줄이 여럿인 응답을 하나로 접을 때 "첫 번째"를 쓰면 줄 순서에 기대는 것이다** — 순서는 데이터
  갱신에 뒤집히고, 그러면 이번엔 반대 성별이 막힌다. 접기 전에 **서로 다른 값이 몇 개인지**부터 세라.
  (같은 파일이 `base_form`/`required_pokemon_form` 에서도 "값이 있다 = 지방이다"로 읽어 이브이를
  날릴 뻔했다 — 이 API 의 조건줄은 *줄마다 다른 질문에 답한다*고 보는 편이 맞다.)
