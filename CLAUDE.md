...
- **여러 조건줄을 우선순위로 접을 때, 그 줄들이 서로 다른 대상의 것일 수 있다 — 그리고 응답은
  대개 누구 것인지를 *말해 준다*. 추측하기 전에 필드부터 찾아라.** PokéAPI 는 한 갈래
  (`evolves_to` 하나)에 조건줄을 여럿 주고, 원종의 것과 지방 모습의 것이 섞여 온다. 파서가 "도구가
  레벨보다 우선"으로 접으면서 원종의 레벨이 사라져 **관동 모래두지가 얼음의돌만 있으면 레벨 1에
  진화**했다(사용자 제보). → 조건은 `requirementRaw`(원종)와 `regionalRequirementRaw`(지방 모습)로
  나눠 담고, 조회할 때 개체의 `region` 으로 고른다(`requirement(for:line:region:)`).

  **한 번 크게 틀렸다: "응답에는 누구 것인지가 안 적혀 있다"고 이 문서에 적어 두고 휴리스틱
  (`levelLivesInAnotherDetail` — 레벨이 다른 줄에 있으면 이 도구는 지방 것)으로 풀었는데, 응답에는
  줄마다 `base_form` 이 처음부터 있었다.** DTO 가 그 필드를 안 읽고 있었을 뿐이다. 휴리스틱은 도구
  줄만 덮어서, 조건이 *친밀도*인 지방 모습에는 아무 방어가 없었다 — **관동 나옹이 알로라 나옹의
  친밀도를 물려받아 레벨 28 대신 친밀도로 진화**했다(사용자 제보). 같은 구멍으로 알로라 식스테일이
  불의돌을, 갈라르 야도킹이 왕의징표석을 요구하고 있었다(전 484갈래 전수 비교로 확인).

  **`base_form` 이 있다고 지방 모습이 아니다.** 39갈래에 값이 있는데 그중 22갈래는 모든 줄이 값을
  갖고, 이브이(여덟 갈래 전부)·피카츄·플라베베·루가루암·호루비·바스라오는 그 값이 *원종 자신*을
  가리킨다. 값의 유무로 가르면 이브이가 조건을 통째로 잃는다 → 판정은 **지방 접미사**로 한다
  (`PokeAPIClient.isRegional`, `Region` 을 단일 소스로 공유). 그리고 지방 줄밖에 없는 갈래(나이킹·
  창치·마임꽁꽁 등 열 갈래)는 갈라내고 끝내면 조건이 0개가 되므로 그 줄을 원종 조건으로도 쓴다.

  **테스트 공백의 정체가 둘이다:** ① 조건 파서 테스트가 전부 **조건줄 하나짜리 픽스처**였다 —
  줄이 둘인 모양을 안 물어봤으니 우선순위가 무엇을 버리는지 물어볼 수조차 없었다. ② 모래두지
  픽스처를 **실제 응답에 있는 `base_form` 없이** 손으로 썼다 — 픽스처가 파서와 같은 공백을 공유해
  둘 다 틀린 채로 통과했다(§외부 로그 포맷은 상위 소스의 writer 로 검증한다 와 같은 부류. 그 규칙이
  로그 파서에만 적용된다고 읽지 마라 — **모든 외부 응답**에 해당한다). 외부 응답을 접는 코드에는
  줄이 여럿인 픽스처를 두고, **그 픽스처는 실제 응답에서 잘라 온다**.
...
- **부수효과를 *동작*이 아니라 *진입점*에 붙이면, 두 번째 진입점이 생길 때 조용히 빠진다.**
  일일퀘스트 계수(`countDailyActivity`)가 `releaseToProfessor`(한 마리) 안에 있었는데, 일괄 보내기
  (`releaseManyToProfessor`)는 **저장 횟수를 줄이려고 일부러 그 함수를 우회**하도록 쓰였다 — 그러면서
  계수까지 같이 빠져 박스에서 스무 마리를 보내도 목표가 0이었다(사용자 제보). 같은 부류로 확정권
  개봉(`redeemEggTicket`)도 알을 놓으면서 뽑기를 안 세고 있었다.
  **테스트 공백의 정체:** `BulkReleaseTests` 는 *돌려주는 값*(포인트 합계·파트너 제외·중복·없는 id·
  빈 목록)을 꼼꼼히 봤지만 *부수효과*는 한 번도 안 물었다. 일괄 경로를 만들 땐 개별 경로가 하던
  일을 전수로 적어 놓고 하나씩 대조하라 — 반환값이 맞다고 같은 일을 한 것이 아니다.
  **계수를 공통 하위 함수로 내리는 것이 항상 답은 아니다.** 알을 놓는 경로는 셋인데(유료 뽑기·
  확정권·파트너가 물어온 알) 뽑기로 세야 하는 것은 앞의 둘뿐이라, `placeEgg` 에 내리면 주운 알까지
  목표를 채운다. **경로마다 의미가 다르면 진입점에 붙이고, 그 판단을 대조군 테스트로 박아라**
  (`testAFoundEggIsNotADraw` 가 정확히 그 유혹을 막는다).
