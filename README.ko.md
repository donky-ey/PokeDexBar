...
3. 🥚 **알을 뽑으세요.** 홈의 **부화칸 줄**에서 재화를 내면 등급이 무작위로 정해진 알을 받습니다 — 커먼 60% · 레어 22% · 에픽 16% · 레전더리 2%, 고를 순 없어요. 뽑는 순간 짧은 연출이 먼저 재생됩니다 — 흰색 폭발은 항상 뜨고, 레어면 하늘색이 한 번 더, 에픽이면 보라색이 한 번 더, 레전더리면 반짝이는 주황색까지 한 번 더 — 폭발이 몇 번 터지는지가 곧 등급이라 결과 글자를 보기 전에 이미 알 수 있어요. 최대 3개가 동시에 부화 중일 수 있고, 슬롯을 사면 6개까지 늘릴 수 있습니다.
...
<tr>
<td width="55%" valign="middle">
<h3>🔎 넘겨 가며 찾지 않아도 돼요</h3>
이름을 치면 아직 열지 않은 뒤쪽 상자까지 박스 전체에서 찾습니다. 이름을 불러오면 일치하는 포켓몬이 결과에 반영되고, 검색어를 바꾸면 첫 결과 페이지로 돌아갑니다. 폼·지방 접두사까지 같이 걸려서 <b>갈라르</b>만 쳐도 그 무리가 다 나오고, 번호로도 찾을 수 있어요. <b>초성</b>도 됩니다 — <b>ㄴㅇ</b> 로 나옹이 나와요. 질의가 전부 자음일 때만 초성 모드라, 글자를 치는 도중에도 평소대로 찾아집니다. 검색은 보기만 거르고 박스를 재배치하지 않으며, 질의를 바꾸면 담아 둔 선택이 비워져요 — 보이는 것이 곧 보낼 것이어야 하니까요.
</td>
<td width="45%" align="center"><img src="assets/box-search.png" width="300" alt="같은 박스를 두 번 검색한 모습 — 초성으로, 그리고 이름 일부로"></td>
</tr>
<td width="55%" valign="middle">
...
<td width="55%" valign="middle">
<h3>🎯 도감이 보답해요</h3>
도감 채우기에 미션이 붙었어요 — 본가의 구조 그대로: 등록 마릿수 고비(10종부터 1000종까지)마다 보상, 세대 하나를 완성하면 <b>레전더리 알 확정권</b> 위에 그 세대의 대표 아이템(메가진화 세대는 메가스톤, 관동·가라르는 다이버섯, 나머지는 반짝이는 사탕), 그리고 1025종 전부를 채우면 <b>무지개 부적</b> — 그 자체로 이로치 확률이 1/32이 되고, 기존 부적이 없어도 지급되며, 이후 이로치 부적을 몇 단계까지 올리든 확률을 한 번 더 좋게 만들어요. 확정권은 가방에 담겼다가 부화칸 아래에서 무료·등급 확정 뽑기로 쓰여요.
</td>
...
<tr>
<td width="55%" valign="middle">
<h3>✅ 박사가 하루 세 가지를 부탁해요</h3>
박사의 제안 바로 아래에 하루면 사라지는 의뢰 셋이 있어요 — 알 뽑기, 부화시키기, 진화시키기, 박사에게 보내기. 보상은 재화가 아니라 <b>박사 포인트</b>예요. 포인트로는 알을 못 사고 토큰으로는 포인트를 못 사서, 매일 하는 일이 상점을 앞지르지 않아요. 셋을 다 받으면 경험치 사탕 하나가 덤으로 한 번. 셋은 그날 날짜와 <b>내 세이브만의 시드</b>로 굴려서 모두의 것이 아니라 내 것이고, 어떤 의뢰도 하루치 수입보다 많이 쓰라고 하지 않아요.
</td>
<td width="45%" align="center"><img src="assets/professor-requests.png" width="300" alt="박사의 의뢰 — 하나는 받았고, 하나는 받을 수 있고, 하나는 진행 중"></td>
</tr>
<tr>
...
미션 옆에 주제별 세트 45개가 있어요 — 전설의 새, 클론의 진실(뮤·뮤츠·메타몽), 이브이 프렌즈, 여섯 세대의 화석 전부, 고대와 미래의 패러독스, <b>세대별 스타터 전부</b>, 동물로 묶은 넷(원숭이·고양이·강아지·동네새), 그리고 <b>전설과 환상 전부</b>에 집이 생겼어요 — 성검사·수호신부터 재앙의 포켓몬과 키타카미의 전승까지. 구성원은 항상 실루엣으로 보여서 뭐가 빠졌는지 한눈에 알 수 있어요. 세트를 완성하면 줄에 메달이 켜지고 보상을 한 번 받아요 — 대부분은 경험치 사탕, 큰 세트는 레전더리 알 확정권. 레지 패밀리는 특별해요: 다섯 기둥을 모으면 <b>레지기가스가 직접 깨어나</b> 박스에 합류해요 — 알에서는 절대 나오지 않는 이 세트만의 보상이고, 이로치 확률도 그대로 굴러가요.
</td>
</tr>
<tr>
<td width="55%" valign="middle">
...
<tr>
<td width="45%" align="center"><img src="assets/draw-slot.png" width="300" alt="부화칸의 세 상태 — 알 옆에 선 10M 짜리 뽑기 칸, 지갑이 모자라 흐려진 같은 칸, 자리가 꽉 차 흐려진 채 줄 아래에 '부화 칸이 꽉 찼어요'가 적힌 모습"></td>
<td width="55%" valign="middle">
<h3>🥚 알이 떨어지는 자리에서 뽑아요</h3>
"뽑기"라고 적힌 칸이 부화칸 줄 맨 앞에 서 있어요 — 쓰는 것과 얻는 것이 같은 자리에 있는 셈이죠. 이 칸은 고정이라 알이 차도 밀리지 않고 스크롤에도 딸려 나가지 않아서, 마우스를 안 옮기고 연달아 뽑을 수 있어요. 재화가 빈 칸 수와 한 줄에 있어서 칸이 흐려졌을 때 지갑 때문인지 자리 때문인지 바로 가려지고, 자리가 다 찼을 땐 줄 아래가 확률 대신 그 사실을 말해 줍니다. 도감 미션에서 받은 알 확정권도 그 자리에 서요 — 개봉은 언제나 알이 태어나는 자리에서 일어나야 하니까요.
</td>
</tr>
<tr>
...
<td width="55%" valign="middle">
<h3>🛒 경제를 위한 상점</h3>
그동안 쓴 토큰이 곧 재화입니다 — 부화 슬롯을 3개에서 6개까지 늘리고, <b>경험치 사탕</b>으로 포켓몬을 키우거나 <b>반짝이는 사탕</b>으로 바로 이로치를 만들 수 있고, 부적 셋 — 부화 이로치 확률을 올리는 <b>이로치 부적</b>, 토큰·사탕으로 얻는 경험치를 올리는 <b>경험치 부적</b>, 토큰으로 얻는 재화를 올리는 <b>행운의 부적</b> — 은 한 번 사고 마는 물건이 아니라 단계를 올리는 물건이에요. 개체 상세 화면에서 <b>메가스톤</b>이나 <b>다이버섯</b>을 쓰면 80종의 폼 중 하나로 모습을 바꿀 수 있어요. 진화 도구와 폼 도구는 일부러 안 팝니다 — 그건 파트너가 물어 와요.
</td>
</tr>
<tr>
<td width="55%" valign="middle">
<h3>🪜 부적은 한 번 사는 물건이 아니라 사다리</h3>
부적마다 계속 올릴 수 있는 단계가 있어요. 값은 한 단계마다 두 배로 뛰는데 효과는 일정하게 붙으니, 단위 이득당 비용이 매 단계 두 배가 돼요 — 상한을 따로 두지 않은 건 값이 곧 상한이기 때문이에요. 각 줄은 무엇을 올리는지, 지금 얼마인지, 다음 단계면 얼마인지를 함께 말해 줘요. 이미 산 사람은 치른 값만큼의 효과를 그대로 가져가요: 예전의 일회성 부적은 4단계에 놓이고, 그건 경험치 2.00배·재화 1.50배·이로치 1/48 — 예전과 같은 숫자예요.
</td>
<td width="45%" align="center"><img src="assets/charm-ladder.png" width="300" alt="같은 경험치 부적의 세 단계 — 미보유 250M, 4단계 4B에 2.00배, 8단계 64B에 3.00배"></td>
</tr>
<tr>
<td width="55%" valign="middle">
...
<td width="55%" valign="middle">
<h3>🎒 모아 온 것을 보는 가방</h3>
상점이 파는 건 일곱 가지뿐입니다. 나머지 105종은 오직 파트너가 물어 와야 생기므로, 가진 것을 보는 자리가 <b>가방</b>이에요 — 소모품은 개수와 함께, 부적은 지금 몇 단계이고 효과가 얼마인지와 함께, 그리고 <b>진화 도구 41종</b>·<b>폼 도구 64종</b>은 얼마나 모았는지 진행도로 보여줍니다. 여기서 쓰지는 않아요: 모든 도구는 특정 포켓몬에게 쓰는 것이라 그 포켓몬의 상세 화면에서 씁니다.
</td>
</tr>
</table>
