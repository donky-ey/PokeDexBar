...
final class ScreenshotGeneratorTests: XCTestCase {
    ...
    func testGenerateScreenshots() throws {
        ...
        // 박사의 제안 + 의뢰 — 이번 릴리스가 새로 만든 **배치**가 이것이다. 포인트를 버는 곳과
        // 쓰는 곳이 한 자리에 붙었다는 게 요점이라, 둘을 따로 찍으면 그 뜻이 안 담긴다.
        // §릴리스 1 하드 게이트가 요구하는 신규 에셋이 이것이다.
    ...
    /// 박사의 제안과 그 아래 의뢰를, 상점에 놓인 순서 그대로. 두 뷰 다 앱이 쓰는 것 그대로다.
    private func professorRequestsBanner(_ fixture: Fixture) -> some View {
        let store = fixture.player
        // 의뢰를 세 가지 상태로 — 받음·받을 수 있음·진행 중. 한 상태만 보이면 줄마다 무엇이
        // 다른지 그림에서 안 읽힌다.
        // 제안 세 칸도 같이 심는다 — 비어 있으면 "오늘의 제안을 준비 중" 한 줄만 나와서,
        // **벌고 쓰는 자리가 붙었다**는 이 그림의 요점이 사라진다. 한 칸은 닫힌 채로 둬
        // 가려진 제안이라는 성질도 같이 보인다.
        store.mutate {
            $0.lastDate = "2026-09-14"
            $0.offerSeed = 42
            $0.researchPoints = 40
            $0.professorOfferDate = $0.lastDate
            $0.professorOffers = [
        ...
        store.countDailyActivity(quests[0].kind, by: quests[0].target)
        store.claimDailyQuest(quests[0])
        store.countDailyActivity(quests[1].kind, by: quests[1].target)
        store.countDailyActivity(quests[2].kind)
        return VStack(alignment: .leading, spacing: 12) {
