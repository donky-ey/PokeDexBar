...
actor PokeAPIClient: PokeProviding {
    ...
    /// 이 조건줄이 **지방 모습의 것인가.** 줄 주인의 슬러그가 지방 접미사(`Region`)로 끝나면
    /// 그렇다 — `meowth-alola`·`sandshrew-alola`·`slowpoke-galar` 처럼.
    ///
    /// **주인은 `required_pokemon_form` 에 적힌다.** 예전에는 `base_form` 만 봤는데 그 필드는
    /// 응답에서 전부 null 이라(576줄 전수 확인) 이 함수가 **한 번도 참이 된 적이 없었다** —
    /// 지방 조건이 26갈래 전부에서 원종으로 샜고, 기본 야도란이 가라두구팔찌를 요구했다(사용자 제보).
    /// 그때의 전수 조사는 옳았는데 필드 이름만 틀리게 기록됐고, 손으로 쓴 픽스처가 `base_form` 에
    /// 값을 넣어 두는 바람에 테스트까지 같은 오해를 공유했다.
    ///
    /// 접미사로 보는 이유는 이 필드가 원종이나 지방과 무관한 폼을 가리키는 경우가 훨씬 많아서다
    /// (이브이가 19줄). 값의 유무로 가르면 이브이가 조건을 통째로 잃는다.
    static func isRegional(_ detail: EvolutionDetail) -> Bool {
        guard let slug = detail.required_pokemon_form?.name ?? detail.base_form?.name else {
            return false
        }
...
struct EvolutionDetail: Decodable, Sendable {
    ...
    /// 문서에 있는 필드라 언젠가 채워질 수 있어 읽기는 하되, 여기에 기대지 않는다.
    let base_form: NamedRef?
    /// **이 조건줄이 누구 것인가.** 줄의 주인이 실제로 적히는 자리다.
    ///
    /// **"값이 있으면 지방 모습"이 아니다.** 전수 확인(576줄 중 81줄에 값): 가장 많은 것이
    /// 이브이(19줄)이고 피카츄·플라베베·루가루암·바스라오·호박귀·사탕훔침처럼 *원종 자신*이나
    /// 지방과 무관한 폼을 가리키는 값이 대부분이다. 유무로 가르면 이브이가 조건을 통째로 잃는다 —
    /// 주인을 가르는 건 값의 유무가 아니라 **지방 접미사**다(`Self.isRegional`).
    let required_pokemon_form: NamedRef?
}
