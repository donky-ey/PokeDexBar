...
3. 🥚 **Draw an egg.** Spend currency in the **hatching row on Home** for an egg of a randomly rolled grade — Common 60% / Rare 22% / Epic 16% / Legendary 2%, never chosen. A short reveal plays first: a white burst, plus a second in pale blue if it's Rare, a third in purple if Epic, and a fourth sparkling orange if Legendary — the number of bursts tells you the grade before the label does. Up to 3 eggs incubate at once, expandable to 6 with a slot upgrade.
...
<tr>
<td width="45%" align="center"><img src="assets/draw-slot.png" width="300" alt="The hatching row in three states: a Draw tile priced at 10M beside the eggs, the same tile dimmed when the wallet is short, and dimmed again with 'Every hatching slot is full' printed under the row"></td>
<td width="55%" valign="middle">
<h3>🥚 Draw the egg where the egg lands</h3>
A labelled Draw tile stands at the head of the hatching row, so the thing you spend on and the thing you get sit in the same place. It is fixed: it never shifts as eggs fill the slots and never scrolls away, so you can draw several in a row without moving the pointer. Your currency reads from the same line as the slot count, which means a dimmed tile tells you which one you're short of — and when every slot is taken, the line under the row says so instead of printing odds you can't act on. Egg tickets from Dex missions stand there too: opening one always happens where an egg is born.
</td>
</tr>
<tr>
